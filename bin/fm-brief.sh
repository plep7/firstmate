...
**Verify isolation before anything else.** Run \`pwd -P\` and \`git rev-parse --show-toplevel\`; a treehouse pool path or an Orca-managed worktree IS the intended disposable worktree and passes this check.
The path check is authoritative: it fails only if the top-level path resolves to firstmate's own project clone (under \`<home>/projects/\`) or to the firstmate home itself; \`git rev-parse --git-dir\` and \`git rev-parse --git-common-dir\` can help inspect the repo, but they do not replace it.
If the top-level path is firstmate's own project clone or the firstmate home itself, STOP - do not branch or commit here - append \`blocked: launched in primary checkout, not an isolated worktree\` to the status file and stop.
