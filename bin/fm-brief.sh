...
# The mode-shaped ship contract itself - extra Setup steps, Rule 1, and the whole
# Definition of done - is generated by bin/fm-ship-contract-lib.sh, which
# bin/fm-promote.sh reuses so a promoted scout lands on the identical contract.
...
# delivery mode, validated above. bin/fm-ship-contract-lib.sh owns all three, so
# bin/fm-promote.sh can rewrite a promoted scout's brief onto the identical
# contract instead of carrying a second copy of this wording. The generated DOD
# opens with the fixed "Delivery contract: mode=<mode>" line that bin/fm-spawn.sh
# checks against its own explicit --mode before launching, and carries the
# PR-description contract for every PR-producing mode.
SETUP2=$(fm_ship_setup_steps "$MODE")
