...
case "$BACKEND" in
  tmux)
    ...
    # the pane cd's into the leased worktree. WT_TARGET carries that stable id for
    # the rename-critical worktree cd below; the persisted window= handle
...
# #134 robustness: only tmux needs a worktree-cd target distinct from $T -
