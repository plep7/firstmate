...
Promoting a scout into `no-mistakes` or `direct-PR` rewrites that task's existing brief onto its new mode's contract, replacing the scout Setup framing, its no-push Rule 1, and the report-based definition of done, so a promoted worker reads one ship contract instead of a scout charter.
[`bin/fm-ship-contract-lib.sh`](../bin/fm-ship-contract-lib.sh) owns those mode-shaped blocks for both scaffolding and promotion, and a `local-only` promotion raises no PR and leaves the brief untouched.
