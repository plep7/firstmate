...
A declared `paused:` line does not exempt a pane from that bound, because a busy pane is working rather than idling on the external wait it declares, so it escalates exactly as an unpaused busy pane would.
...
For an ordinary crew that has stopped, the normal-mode watcher trusts an unchanged `paused:` or durable `captain-held` verb on its own and applies that same bounded cadence regardless of agent liveness, so a live crew idling at an external gate settles into the pause instead of flip-flopping back to ordinary stale wakes.
Only a crew that is provably working or an endpoint the backend authoritatively reports as `missing` overrides the declared pause, so a genuinely vanished pane still escalates on the ordinary wedge cadence, and the secondmate idle-endpoint exemption is unchanged.
