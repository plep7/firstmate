...
Like zellij's equivalent, this primitive is no longer on the spawn path - `fm-spawn.sh` captures the leased worktree path directly from non-interactive `treehouse get --lease` (see `docs/architecture.md` "Worktrees, not branches in your checkout") - and is kept as a verified backend fact exercised by the cmux tests.
