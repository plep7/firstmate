...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates, the same bounded cadence for a declared pause whose endpoint the backend reports missing, and how often a declared pause re-reads authoritative crew state instead of reusing its last verdict; stale panes whose crew is not provably working surface immediately unless they declare the pause verb
