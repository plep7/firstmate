...
| Send + submit atomically | `herdr pane run <pane> <command>` | Runs and submits a command in one call; used for the two fixed spawn-time commands (the `cd` into the leased worktree, the `GOTMPDIR` export) exactly where tmux used one `send-keys ... Enter` call. |
...
- **RESOLVED: worktree-discovery isolation guard's symlinked-project-prefix false refusal.** Originally discovered while building the runtime-backend-auto-detection real smoke test (`tests/fm-backend-autodetect-smoke.test.sh`), which needed a scratch project.
  ...
  The observed-pane-cwd canonicalization described above has since been retired along with the pane-cwd poll itself: `fm-spawn.sh` now captures the leased worktree path directly from non-interactive `treehouse get --lease` (see `docs/architecture.md` "Worktrees, not branches in your checkout"), while `PROJ_ABS_REAL` still anchors `validate_spawn_worktree`'s primary-vs-worktree comparison, and the regression test drives the lease-based spawn.
