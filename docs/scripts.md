...
| `fm-pr-contract-lib.sh`  | Single owner of the generated PR-description contract carried by every PR-producing task |
| `fm-ship-contract-lib.sh`| Single owner of the mode-shaped ship contract (Setup steps, Rule 1, Definition of done) shared by scaffolding and promotion |
...
| `fm-promote.sh`          | Promote a scout task in place to a protected ship task with an explicit delivery mode, reconciling its brief to that mode's ship contract |
