...
| `fm-ensure-agents-md.sh` | Ensure a project's canonical memory file (`AGENTS.md`+`CLAUDE.md` symlink, or an existing `CLAUDE.md` left as-is) carries the canonical self-governance section |
...
| `nm-pr-format`           | Reformat a no-mistakes-opened PR body into the shared team template (the full-pipeline fallback flow's post-processing step) |
