...
1. **An authenticated browser session is reachable.**
   `chrome-devtools-axi` must reach a Chrome session that is already logged into GitHub (via `CHROME_DEVTOOLS_AXI_AUTO_CONNECT` against a real running Chrome, a `CHROME_DEVTOOLS_AXI_USER_DATA_DIR` profile with an existing GitHub session, or an equivalent already-authenticated session your environment provides).
   A freshly launched isolated Chrome profile is **not** authenticated by default — confirm this concretely (see Step 2) rather than assuming it.
