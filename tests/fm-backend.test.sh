...
OLD_BIN_UNCHANGED_SIBLINGS="fm-gate-refuse-lib.sh fm-guard.sh fm-lock-lib.sh fm-tangle-lib.sh fm-tmux-lib.sh fm-composer-lib.sh fm-marker-lib.sh fm-wake-lib.sh fm-classify-lib.sh fm-ff-lib.sh fm-config-inherit-lib.sh fm-tasks-axi-lib.sh fm-supervision-lib.sh fm-supervision-instructions.sh fm-decision-hold.sh fm-project-mode.sh fm-harness.sh fm-crew-state.sh fm-backend.sh"
...
make_spawn_fakebin() {  # <dir> <fake-worktree-path> -> echoes fakebin dir
  # Serves BOTH acquisition shapes so the same fakebin drives an old
  # (pre-lease) build and the current build side by side: the old build sends
  # interactive `treehouse get` into the pane and polls `#{pane_current_path}`
  # (served by the tmux stub below); the current build runs
  # `treehouse get --lease --lease-holder <id>` itself and captures its
  # stdout (served by the treehouse stub below). Both resolve to the same $wt.
...
#
# docs/herdr-backend.md "Known gaps": PROJ_ABS is a logical `cd && pwd`, so it
# can carry a symlinked path component (e.g. macOS's /tmp -> /private/tmp)
# that differs string-for-string from a physically-resolved path even though
# both name the same directory. validate_spawn_worktree compares the leased
# worktree's PHYSICAL path (`pwd -P`) against PROJ_ABS_REAL (also physically
# resolved) for exactly this reason. Since fm-spawn.sh now captures
# `treehouse get --lease`'s printed path directly instead of polling a pane's
# cwd, the historical "stale first poll" race this test used to model against
# a real backend read cannot recur; what remains worth pinning is that a
# project reached through a symlinked prefix still spawns cleanly.
test_spawn_symlinked_project_prefix_avoids_false_refusal() {
  local real_root link_root proj wt id fb data state config log out rc
  real_root="$TMP_ROOT/symlink-real"; link_root="$TMP_ROOT/symlink-link"
  mkdir -p "$real_root"
  ln -s "$real_root" "$link_root"
  proj="$link_root/proj"
  wt="$TMP_ROOT/symlink-wt"
  id="spawnsymlink1"
  fm_git_pooled_worktree "$real_root/proj" "$wt"
  fb=$(make_spawn_fakebin "$TMP_ROOT/symlink-fake" "$wt")
  data="$TMP_ROOT/symlink-data"
  mkdir -p "$data/$id"
  printf 'test brief content\n' > "$data/$id/brief.md"
  state="$TMP_ROOT/symlink-state"; config="$TMP_ROOT/symlink-config"
  mkdir -p "$state" "$config"
  log="$TMP_ROOT/symlink-spawn.log"
  ...
  assert_contains "$out" "worktree=$wt" \
    "fm-spawn.sh did not resolve a symlinked-prefix project to its leased worktree"
