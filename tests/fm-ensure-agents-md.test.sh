...
test_neither_file_creates_claude_md_not_agents_md() {
  local repo claude
  ...
  assert_present "$claude" "CLAUDE.md was not created"
  assert_absent "$repo/AGENTS.md" "AGENTS.md was created for a project with neither file"
  [ ! -L "$claude" ] || fail "CLAUDE.md was created as a symlink for a project with neither file"
  ...
  pass "fm-ensure-agents-md.sh: neither file present creates CLAUDE.md, not AGENTS.md"
}

test_existing_claude_md_is_left_as_is() {
  local repo claude out before
...
EOF
  ...
  out=$("$ROOT/bin/fm-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "fm-ensure-agents-md.sh failed for existing CLAUDE.md"
  assert_contains "$out" "unchanged:" "existing CLAUDE.md was not reported unchanged"
  assert_absent "$repo/AGENTS.md" "AGENTS.md was created for a project with a real CLAUDE.md"
  [ ! -L "$claude" ] || fail "CLAUDE.md was converted to a symlink"
  [ "$(cat "$claude")" = "$before" ] || fail "existing CLAUDE.md content was modified"
  pass "fm-ensure-agents-md.sh: existing CLAUDE.md is left as-is, never promoted to AGENTS.md"
}

test_existing_claude_md_without_trailing_newline_is_untouched() {
  local repo claude
  ...
  "$ROOT/bin/fm-ensure-agents-md.sh" "$repo" >/dev/null 2>&1 \
    || fail "fm-ensure-agents-md.sh failed for newline-less CLAUDE.md"
  cmp -s <(printf '# Existing agent memory\n\nRun tests with make test.') "$claude" \
    || fail "newline-less CLAUDE.md was modified"
  pass "fm-ensure-agents-md.sh: newline-less existing CLAUDE.md is left byte-identical"
}
...
test_dangling_correct_claude_symlink_still_creates_agents_md() {
  local repo agents
  ...
  agents="$repo/AGENTS.md"
  "$ROOT/bin/fm-ensure-agents-md.sh" "$repo" >/dev/null 2>&1 \
    || fail "fm-ensure-agents-md.sh failed for a dangling CLAUDE.md -> AGENTS.md symlink"
  assert_present "$agents" "AGENTS.md was not created behind an existing correct-target symlink"
  [ -L "$repo/CLAUDE.md" ] || fail "CLAUDE.md symlink was replaced instead of kept"
  pass "fm-ensure-agents-md.sh: a dangling but correctly targeted CLAUDE.md symlink still creates AGENTS.md"
}
